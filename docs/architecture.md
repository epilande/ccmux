...
Desktop notifications are opt-in (`notifications.enabled`) and edge-triggered on `waiting` / `finished` transitions. Notifications are _actionable_: a permission wait carries **Approve** / **Deny** buttons, and a question wait or a `finished` (idle) notification carries an inline **Reply**, all of which reach the agent's pane without a context switch.
...
**`POST /notification-action` + shared handler (`notification-action.ts`).** The macOS helper is a one-shot CLI app: it posts the notification and exits, and macOS relaunches it on a button press to POST `{ sessionId, action, statusChangedAt, userText? }` back. Both that HTTP route and the Linux D-Bus `ActionInvoked` dispatch funnel into one in-process `handleNotificationAction`, so the safety rules can't drift between platforms. An action button types into a live pane, so every mutating action is gated by two orthogonal checks: the action must be whitelisted (`default`, `approve`, `deny`, `answer`; `dismiss` is deliberately absent, a dismissed notification posts nothing), the session must still exist, and then (1) the `statusChangedAt` staleness token the notification was stamped with must still match the session's, and (2) the pure `resolveActionPlan(action, session, agentDef)` must find the action legal in the session's LIVE state and return how to run it. The state matrix: on a `permission` wait approve/deny send the agent's key map, and `answer` is a deny-with-feedback when the agent sets `permissionReplyPrelude` (its keys cancel the prompt before the reply text lands as the next message); on a `plan_approval` wait (ExitPlanMode) approve/deny send the SEPARATE `planApprove`/`planDeny` keys and `answer` replies when the agent sets `planReplyPrelude`; on a `question` wait `answer` replies when the agent sets `replyOnQuestion` AND an `answerPrelude` to cancel the picker with (all three reply rows gate on their prelude for the same reason: the picker ignores typed text, so with no cancel key the Enter would select whichever option is highlighted); on an `idle` (finished) session `answer` replies when it sets `replyOnFinished`; anything else (e.g. approve on idle) is illegal and returns `null`. Plan waits are matched by a shared `isPlanApprovalWait` predicate (exported from `notification-action.ts`, reused by the notifier context) and checked BEFORE the plain-permission rows, because a LIVE hook-tracked plan wait is stored as a `permission` (the marker wins the cascade as `waiting_permission`; the `ExitPlanMode` tool name reaches `pendingTool` only from the log). That ordering is safety-critical: it makes Approve send the plain-approve digit (Claude: `2`), never the permission `1`, which at the ExitPlanMode picker enables auto mode. Either check failing sends no keystroke, returns 409, and fires a fresh "state changed" re-notification so the user never believes a press landed that didn't. The token pins a status edge, not an attention flip: a `permission` → `question` change WITHIN `waiting` does not bump `statusChangedAt` (only status edges do, `SessionManager.updateSession`), so the token alone can't catch it. Benign for Claude, since both waiting reply variants cancel with the same `Escape` prelude. Approve/deny keystrokes come from the per-agent `AgentDef.notificationActions` map (Claude: approve = `1`, deny = `Escape`); a missing map is itself treated as stale. A reply first sends the plan's prelude keys plus a settle delay, then the text: the `permission` reply sends `permissionReplyPrelude`, the `plan_approval` reply sends `planReplyPrelude`, and the `question` reply sends `answerPrelude` (all `Escape` on Claude, cancelling the prompt/picker back to the composer, which ignores typed text), while an idle reply deliberately sends NO prelude, since Escape at Claude's idle composer would clear a typed draft. A reply whose text begins with `/` or `!` is sent with one leading space so it never trips a composer mode instead of reaching the agent: `/` opens the slash-command palette, and `!` is Claude's shell mode, where the text runs as a shell command with no permission prompt (verified on 2.1.211; `#` is NOT a mode trigger there and types through). Reply text is sanitized to a single control-char-free line and length-capped (`MAX_NOTIFICATION_REPLY_CHARS`) before it reaches `ccmux send`.

**Press-time pane authority (beyond the token gate).** The stored plan/permission classification is unreliable in BOTH directions and the staleness token can't catch it (status stays `waiting` throughout): a live plan wait USUALLY arrives with `pendingTool` null (the marker reports `waiting_permission` with a null tool AND ExitPlanMode's `tool_use` is frequently deferred out of the JSONL during the wait, like a permission-gated one), and a permission wait right after a plan wait can retain a stale `ExitPlanMode` `pendingTool` (the cascade evaluator carries the tool name forward). Because the null-`pendingTool` window is the COMMON case, a veto-style guard would 409 nearly every real plan approval. So for any approve/deny/answer press on a waiting permission/plan wait by an agent whose plan picker differs from its permission prompt (`planApprove` defined), the handler captures the pane, classifies it with `classifyClaudePromptPane` (`pane-classify.ts`, bottom-anchored on the last prompt terminator so a stale plan footer can't shadow a fresh prompt below it), and passes the result into `resolveActionPlan` as the AUTHORITATIVE wait type: the pane decides whether Approve keys the plan `2` or the permission `1`, and it drives `answer`'s prelude choice (plan vs permission Reply) too, not the stored classification. For an approve/deny press a null classification (or a capture failure) means no active prompt is on screen and the press 409s, fail CLOSED, because keying `1` at a plan picker enables auto mode and `2` at a Bash prompt is the persistent grant. `answer` skips only that fail-closed 409: an AskUserQuestion picker classifies as null, so it falls back to the stored wait type rather than refusing. A reply that cancels a prompt the handler CLASSIFIED then re-captures the pane after the prelude's settle and requires `classifyClaudePromptPane` to return null (the prompt provably gone) before typing, 409ing fail CLOSED if it is still live or the capture fails. The prelude is otherwise fire-and-forget (`sendKey` resolving true only means tmux accepted the keystroke, not that the TUI acted on it), and an Escape immediately followed by printable bytes can be read as ONE escape sequence (Alt+char), so the cancel never lands, the still-live picker swallows the text, and the Enter selects the highlighted option: a deny-with-feedback press would silently APPROVE and report 200 (reproduced on 2.1.212). The settle makes that rare, not impossible, so the outcome is verified rather than assumed. And before ANY send, a liveness guard checks `#{pane_current_command}`: the reconciler keeps a dead agent's session as idle with its pane still bound, so if the foreground process is a shell a Reply would EXECUTE as a command, and the press 409s (also fail CLOSED on a query miss). The notifier's offer side runs the SAME `classifyClaudePromptPane` over the live pane, authoritative in the same way, so its buttons/subtitle match what is on screen (fail-open there, falling back to the predicate; the press-time handler is the enforcement point).
...
**Plan-wait reclassification.** A LIVE plan wait is usually stored as a `permission` with a null `pendingTool`: Claude's `Notification` hook fires for ExitPlanMode with `state: waiting_permission` and `pending_tool: null`, the marker wins the cascade, and the `ExitPlanMode` tool name that would otherwise reach `pendingTool` from the log is frequently deferred (verified on Claude Code 2.1.211). So `buildNotificationContext` does not trust the stored type for a Claude permission/plan wait: it captures the pane and lets `classifyClaudePromptPane` decide, promoting to `plan_approval` (returning `reclassifyAs: "plan_approval"`) whenever the pane shows the plan picker, with the `isPlanApprovalWait` predicate kept only as the capture-failure fallback. Without this a plan notification would render the permission Approve/Deny and its Approve key (`1`) would enable auto mode at the real picker; with it the delivery carries the plan actions (Approve = `2`), the plan Reply, and the "Plan ready for review" subtitle.
