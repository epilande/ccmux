...
**Context bodies (`notify-context.ts`).** So a notification shows _what_ it is waiting on, `buildNotificationContext` reads the tail of the session's transcript at notify time: a `permission` wait renders the pending `tool_use` plus its most salient input (e.g. the `Bash` command, the `Edit` file path), a `question` wait the last assistant message. Claude-only in v2 (others return `null`); the text is agent-derived so it is stripped of control characters (keeping `\n`, which the helper renders verbatim) and clamped to a few glanceable lines. Fail-open: any parse/read error keeps the base body.
