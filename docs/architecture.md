...
**Press-time guards (beyond the token gate).** The stored plan/permission classification is unreliable in BOTH directions and the staleness token can't catch it (status stays `waiting` throughout): a live plan wait can arrive with `pendingTool` null (a lost log stamp), and a permission wait right after a plan wait can retain a stale `ExitPlanMode` `pendingTool` (the cascade evaluator carries the tool name forward). So for an agent whose plan picker differs from its permission prompt (`planApprove` defined), an approve/deny keys send re-reads the pane with `classifyClaudePromptPane` (`pane-classify.ts`, bottom-anchored on the last prompt terminator so a stale plan footer can't shadow a fresh prompt below it) and 409s unless the pane matches the type being keyed. And before ANY send, a liveness guard checks `#{pane_current_command}`: the reconciler keeps a dead agent's session as idle with its pane still bound, so if the foreground process is a shell a Reply would EXECUTE as a command, and the press 409s. Both guards fail CLOSED (a capture/query miss also 409s), the opposite of the enrichment paths' fail-open, because a dropped press is recoverable but a wrong grant or a command run in the shell is not. The notifier's offer side runs the SAME `classifyClaudePromptPane` over the live pane, so its buttons/subtitle match what is actually on screen (fail-open there, falling back to the predicate; the press-time guard is the enforcement point).
...
**Plan-wait reclassification.** The same delivery-time machinery promotes a `permission` wait whose `pendingTool` is `ExitPlanMode` to `plan_approval` (via the shared `isPlanApprovalWait` predicate, matched BEFORE the permission branch in `buildNotificationContext`), returning `reclassifyAs: "plan_approval"`. A LIVE plan wait is stored as a permission because Claude's `Notification` hook fires for ExitPlanMode with `state: waiting_permission` and `pending_tool: null` and the marker wins the cascade (verified on Claude Code 2.1.211), so without this promotion a plan notification would render the permission Approve/Deny and its Approve key (`1`) would enable auto mode at the real picker. With it, the delivery carries the plan actions (Approve = `2`), the plan Reply, and the "Plan ready for review" subtitle.
