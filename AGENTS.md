...
Built-in agents: `src/lib/agents.ts`. Each `AgentDef` includes: `processMatch`, `commandPatterns`, `terminalRules`, `errorRules`, `executable`, `resumeCommand`, `promptCommand`, `forkCommand`, `modelFlag`, `sessionFilePattern`, `versionCommand`, `hooks`, `invokeMode`, `readyPattern`.
