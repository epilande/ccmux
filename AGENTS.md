...
- **Worktrees panel** (`W`, or the group context menu; `WorktreesPanel.tsx`) - Three-phase read: the local worktree list paints instantly, the prune scan merges in by path afterwards, and the repo's open PRs arrive as their own section, which is why the cursor is key-tracked (a PR row's key is synthetic) and scrolling is measured in visual lines. Its row presentation and removal flow are load-bearing responses to live testing; read [`docs/architecture.md#worktrees-panel`](docs/architecture.md#worktrees-panel) before touching it.
