...
- **Source picker** (`N`, persistent Start view in the picker and compact overlay in the sidebar; `SourcePicker.tsx`) - One filterable list of open PRs AND open issues across the current repo scope. Enter is its verb on a row: it feeds the new-session dialog's PR and ISSUE modes, or goes to the checkout that already holds the row. One list rather than two tabs because the filter is the point (one word reaches both kinds); NAV mode by default with `/` for the filter; repo headers are selectable for Enter-collapse and Space-mark; the layout counts them as one line. A PR is proved checked out by SHA and an issue by its ccmux-derived `issue-<n>` name, and that asymmetry is deliberate. Its two cursor effects are subtle in the same way and both have regression tests: the scroll effect must read every signal BEFORE its `listBox` guard (the ref is not reactive, so guarding first leaves a dependency-less effect Solid never re-runs, and the list silently stops scrolling), and the re-seed effect must HOLD a seeded key whose own source has not answered yet, since the three reads land independently. Read [`docs/architecture.md#source-picker`](docs/architecture.md#source-picker) before touching it.
- **Worktrees panel** (`W`, persistent picker view or compact sidebar overlay; `WorktreesPanel.tsx`) - The local worktree list paints instantly and the prune scan merges classification by path afterwards. PRs and issues live in Start. Cursor and marks are key-tracked; scrolling counts visual lines. `h`/`l` cycle picker views, `s` changes scope, and removal has a separate dirty-file consent question before final confirmation. Read [`docs/architecture.md#worktrees-panel`](docs/architecture.md#worktrees-panel) before touching it.
...
Built-in agents: `src/lib/agents.ts`. Each `AgentDef` includes: `processMatch`, `commandPatterns`, `terminalRules`, `errorRules`, `executable`, `resumeCommand`, `promptCommand`, `forkCommand`, `modelFlag`, `sessionFilePattern`, `versionCommand`, `hooks`, `invokeMode`, `readyPattern`, `summaryTitle`.
