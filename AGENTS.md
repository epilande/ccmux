...
`Enter` is context-sensitive (jump to the row's session, else `openNewSession` with `existingWorktree` for a linked worktree or a plain `cwd` for the main checkout), which is why removal moved to `x`.
