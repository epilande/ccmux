...
`columnHeader` defaults to `auto`: the header shows for the flat `none` grouping, where the list reads as a table, and stays off for the grouped modes, whose group lines sit between a header and the rows it labels. `ccmux config set columnHeader always` or `never` overrides that.
