...
The picker's first line is a titled rule naming the three views, Sessions, Worktrees, and Start, with the active one marked by a bold `━━` stub (`━━ Sessions ·3 ── Worktrees ── Start ───`). Each view's count rides after the name (`Sessions ·3`, `Worktrees ·2~` when the last lookup failed). <kbd>h</kbd>/<kbd>l</kbd> cycle views; <kbd>s</kbd> switches between all repos and the cursor's repo. <kbd>W</kbd> and <kbd>N</kbd> jump to Worktrees or Start already narrowed. The sidebar keeps compact overlays for those shortcuts. The right end of that line is `scope: all repos` or `scope: <repo>`, plus a few signals that appear only when something needs noticing: `● reconnecting` when the daemon connection drops, `▲ degraded` when daemon scans keep failing, `N invoking` while `ccmux invoke` workers run, and `active` while <kbd>f</kbd> hides idle sessions (the Sessions count then reads `·n/total`, as it does under a search). Narrow terminals drop the scope first; the connection and degraded marks are never dropped.
...
`ccmux worktree prune` uses the same classification:
...
| Refresh               | <kbd>R</kbd>                                                                       | Refresh cached facts and reconnect to the daemon                                                                       |
| Kill / remove         | <kbd>x</kbd>                                                                       | Act on marked rows, otherwise the cursor row / header's group                                                          |
| Kill displayed        | <kbd>X</kbd>                                                                       | Kill displayed sessions in the current view and scope, excluding collapsed groups, after confirmation                                                                                              |
| Fork session          | <kbd>F</kbd>                                                                       | Branch the conversation into a pane of its own, leaving the original running                                           |
| Worktrees             | <kbd>W</kbd>                                                                       | Jump to Worktrees narrowed to the cursor's repo                                                                        |
| Start from PR/issue   | <kbd>N</kbd>                                                                       | Jump to Start narrowed to the cursor's repo                                                                            |
...
| Change view           | <kbd>h</kbd> / <kbd>l</kbd>                                                        | Previous / next view                                                                                                   |
| Scope                 | <kbd>s</kbd>                                                                       | All repos / cursor's repo                                                                                              |
| Collapse/expand       | <kbd>Enter</kbd> on a header                                                       | Toggle that group                                                                                                      |
| Mark                  | <kbd>Space</kbd>                                                                   | Mark row / header's group                                                                                              |
| Mark all / clear      | <kbd>a</kbd> / <kbd>A</kbd>                                                        | All visible rows / clear marks                                                                                         |
...
| Hide idle             | <kbd>f</kbd>                                                                       | Toggle hiding idle sessions (the strip shows `n/total` and `active`); the `needs attention` band stays visible               |
...
| `attentionBand`              | `true`, `false`                                                              | `true`             | Pin waiting sessions in a `needs attention` band at the top of the list                                                            |
...
| `columnHeader`               | `auto`, `always`, `never`                                                    | `auto`             | Column-header line above the picker list (`auto` shows it for the flat `none` grouping only; see [Column header](#column-header))  |
...
Waiting sessions move into a pinned `needs attention` band above the groups, oldest wait first. They appear once, retain repo and tmux session identity, and return to their group when the wait ends. The band disappears when empty and stays expanded through collapse-all. With grouping off, a `sessions` header ends the band so the rows below it do not read as waiting. Search still filters sessions; `f` never removes a waiting row. Each header counts its own remaining rows. To keep waiting sessions in their own groups instead, run `ccmux config set attentionBand false`; it applies the next time the picker or sidebar opens.
...
The picker can draw one line of column labels above the session list. It is derived from the resolved `columns` layout at the current width, so it follows your reorders, dropped fields, and breakpoint mode changes. Only cells with a fixed position get a label: the fixed-width cells on the right (`agent`, `version`, `pane`, `time` as `age`), the fixed-width cells at the start of the left side (`status`), and the first flexible cell (`project`) at its fixed start. A flexible field placed on the right, and anything to its left, is unlabeled because its position changes per row. Labels that would not fit a cell (the two-column short agent code) are left blank rather than cut. The line is picker-only, hidden below the `md` breakpoint, and never drawn in the sidebar.

`columnHeader` defaults to `auto`: the header shows for the flat `none` grouping, where the list reads as a table, and stays off for the grouped modes, whose group lines sit between a header and the rows it labels. A flat list also hides it while the `needs attention` band is showing, for the same reason. `ccmux config set columnHeader always` or `never` overrides that.
...
Defaults: `row1.left` is `index, status, project` (status grows from an icon to `short` at `sm` and `full` at `md`, and respects configured modes). `row1.right` cascades by breakpoint: just `pane` below `xs`, then `agent:short, pane` at `xs`, `agent:short, pane, time` at `sm`, and `agent:full, version, pane, time` at `md`+. The `summary` and `pr` cells are configured on `row2`, but `promptDisplay` (default `inline`, cycled live by <kbd>p</kbd>) controls how they render: `inline` flattens them onto `row1` so each session stays a single line, `row2` gives the subtitle its own line with `pr` at the right edge, and `off` hides both. Sessions with no prompt stay single-line in `inline` mode; in `row2` mode the second line still appears when another row-2 field (such as an open PR) has data.

Sidebar defaults differ to fit the narrow rail: `row1` is `index, status, project` with `pr:short, agent:short` on the right (PR stays visible even with the prompt hidden), and `row2` is `summary` / `time` (a lone `time` never earns the row; it rides along when some other field has data). The 30-col rail has no room to inline, so the sidebar always uses the two-row layout (`inline` behaves like `row2`). A waiting row also gets a fitted tmux target line, so its session name survives at 30 columns. Override these under the `sidebar.columns` key in `~/.config/ccmux/ccmux.json` (e.g. `"sidebar": { "columns": { "row2": { "left": ["pane"] } } }` to bring the pane target back).
