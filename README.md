...
Actionable Approve/Deny buttons work for **Claude Code**, **OpenCode**, and **Codex**; other agents get click-to-jump for now. Inline **Reply** is Claude Code only. Approve/Deny work on macOS and Linux; inline reply needs a notification server that advertises it (always on macOS, varies on Linux).
